...
//Definition eines Rechtecks: hat gerade kanten und ist vollständig mit einer Farbe gefüllt
...
uint8_t static isRect(int32_t x, int32_t y, int32_t rectHeight, int32_t rectWidth, struct tagBitMap8Bit *picture8Bit) {
    uint8_t color = picture8Bit->pixel[y][x];
    //printf("x: %d \t y: %d \t height: %d \t width: %d \t ", x, y, rectHeight, rectWidth);
    /*
    ...
    */
    ...
    for (int32_t i = y; i <= rectHeight; i++)
    {
        if (x > 0) // linke Seite testen --> muss Seite nicht testen falls am linken BildRand
        {
        ...
        if ((rectWidth + 1 < width)) //rechte Seite testen -- muss Seite nicht testen falls am rechten Rand
        {
    ...
    {
        if (y > 0) // untere Seite Testen --> nicht testen falls unterer BildRand
        {
        ...
        if ((rectHeight + 1 < height)) // untere Seite testen --> nicht testen falls unterer Rand
        {
